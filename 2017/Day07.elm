...
import Parser exposing ((|.), (|=), Parser, end, ignore, int, keep, keyword, oneOf, oneOrMore, repeat, succeed, symbol, zeroOrMore)
import Set
...
type Node
    = Node Program (List Node)


main : Html msg
main =
    { part1 = (\(Node p _) -> p.name) tree
    , part2 = tree |> mapToTotalWeights -- elm-format output, fold away good nodes, see that ycbgx needs to slim down by -5
    }
        |> toString
        |> text
...
mapToTotalWeights (Node program children) =
    let
        weightedChildren =
            List.map mapToTotalWeights children

        additionalWeight =
            List.sum <| List.map weightOf weightedChildren
    in
    Node { program | weight = program.weight + additionalWeight } weightedChildren


{-| parsing the input to an intermediary structure of `List (Program, List String)`, where the `List String` are the names of the children.
This way we can build up our tree faster, as we can find out efficiently who is root by just comparing all names to all names of children.
-}
tree : Node
tree =
    let
        -- PARSERS
        -- yes, using Parser seems a little overkill. but i wanted to learn something
        parseData s =
            let
                spaces : Parser ()
                spaces =
                ...
                programParser : Parser ( Program, List String )
                programParser =
                    let
                        program =
                            succeed Program
                ...
                inputParser : Parser (List ( Program, List String ))
                inputParser =
                    succeed identity |= repeat oneOrMore programParser
            in
            case Parser.run inputParser s of
                Ok list ->
                    list
        ...
        -- TRANSFORMATION
        root : List ( Program, List String ) -> String
        root input =
            let
                allPrograms =
                    Set.fromList <| List.map (Tuple.first >> .name) input

                programsOnDiscs =
                    Set.fromList <| List.concatMap Tuple.second input
            in
            Set.diff allPrograms programsOnDiscs
                |> Set.toList
                |> List.head
                |> Maybe.withDefault "No root found..."

        toTree : List ( Program, List String ) -> Node
        toTree input =
            let
                dict =
                    input
                        |> List.map (\(( p, children ) as val) -> ( p.name, val ))
                        |> Dict.fromList
                ...
                find key =
                    case Dict.get key dict of
                        ...
                        Nothing ->
                            Debug.crash "your input says there's an undefined program on a disc"

                build ( program, children ) =
                    Node program (List.map (find >> build) children)
            in
            build <| find (root input)
    in
    """jlbcwrl (93)
