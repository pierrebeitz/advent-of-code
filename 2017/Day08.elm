...
import Parser exposing ((|.), (|=), Parser, end, ignore, int, keep, keyword, map, oneOf, oneOrMore, repeat, succeed, symbol, zeroOrMore)


main : Html msg
main =
    { part1 = processInstructions input |> Tuple.first |> Dict.values |> List.maximum
    , part2 = processInstructions input |> Tuple.second
    }
        |> toString
        |> text
...
type
    Operation
    --            abc      inc     42
    = Operation Register Operator Int
...
processInstructions : List Instruction -> ( Dict String Int, Int )
        ...
        applyOperation (Operation reg op int) dict =
            let
                oldVal =
                    get reg dict
            in
            Dict.insert reg (op oldVal int) dict

        processInstruction (Instruction op q) ( dict, max ) =
            if isQueryMatching q dict then
                let
                    newDict =
                        applyOperation op dict

                    currentMax =
                        newDict |> Dict.values |> List.maximum |> Maybe.withDefault 0
                in
                ( newDict
                ...
                ( dict, max )
    in
    List.foldl processInstruction ( Dict.empty, 0 ) input
...
parse input =
    let
        ...
        instructionList =
            repeat zeroOrMore instruction
    in
    case Parser.run instructionList input of
        Ok list ->
            list
